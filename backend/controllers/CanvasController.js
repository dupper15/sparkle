...
const addComponentToCanvas = async (req, res) => {
  try {
    ...
    if (
      ...
      !mongoose.isValidObjectId(componentId)
    ) {
      return res.status(400).json({
        status: "ERROR",
    ...
    const response = await CanvasService.addComponentToCanvas(
      canvasId,
      componentId
    );
...
const removeComponentFromCanvas = async (req, res) => {
  try {
    ...
    const response = await CanvasService.removeComponentFromCanvas(
      canvasId,
      componentId
    );
    return res.status(response.status === "OK" ? 200 : 400).json(response);
    ...
    return res.status(500).json({
      status: "ERROR",
      message:
        "An error occurred while removing the component from the canvas.",
      error: e.message,
    });
  }
};
