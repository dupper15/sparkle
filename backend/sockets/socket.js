...
const { sendMessage, getMessage, getUserName } = require('../services/MessageService');
...
const initializeSocket = async (server, mongoUri) => {
	...
	io.on('connection', (socket) => {
		...
		socket.on('select-component', async ({ id, userId1, roomId }) => {
			...
			socket.to(roomId).emit('update-select-component', { id, userId1, userName });
		});
		socket.on('deselect-component', ({ componentId, userId, roomId }) => {
			if (!componentId || !userId || !roomId) return;
			socket.to(roomId).emit('update-deselect-component', { componentId, userId });
		});
