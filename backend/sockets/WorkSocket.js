const WorkSocket = (socket, io) => {
	...
	socket.on('textFontFamilyChanged', ({ componentId, fontFamily, roomId }) => {
		...
		socket.to(roomId).emit('textFontFamilyChanged', { componentId, fontFamily });
	});
	...
	socket.on('textFontWeightChanged', ({ componentId, fontWeight, roomId }) => {
		...
		socket.to(roomId).emit('textFontWeightChanged', { componentId, fontWeight });
	});
	...
	socket.on('textDecorationLineChanged', ({ componentId, textDecorationLine, roomId }) => {
		if (!roomId || !componentId || !textDecorationLine) return;
		socket.to(roomId).emit('textDecorationLineChanged', { componentId, textDecorationLine });
	});
	...
	socket.on('componentHorizontalFlipChanged', ({ roomId, componentId, horizontalFlip, componentType }) => {
	...
	socket.on('componentVerticalFlipChanged', ({ roomId, componentId, verticalFlip }) => {
		socket.to(roomId).emit('componentVerticalFlipChanged', { componentId, verticalFlip });
	});
