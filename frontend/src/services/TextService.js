...
const TextService = {
  async createText(textData) {
    const response = await axios.post(
      `${import.meta.env.VITE_API_KEY}/text/create`,
      textData
    );
  ...
  async updateText(id, updatedData) {
    return axios
      .put(`${import.meta.env.VITE_API_KEY}/text/update/${id}`, updatedData)
  ...
  async deleteText(id) {
    return axios
      .delete(`${import.meta.env.VITE_API_KEY}/text/delete/${id}`)
      .then((response) => response.data)
      .catch((error) => {
  ...
  async updateTextFontFamily(fontFamily, textId) {
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_API_KEY}/text/update/${textId}`,
        { fontFamily }
      );
      return response.data;
    } catch (error) {
