				...
				<div className='chat-image avatar'>
					<div className='w-8 sm:w-10 rounded-full'>
						{message.senderName !== 'SparkleBot' ? (
							...
							<img alt={chatbotAvatar || 'Avatar'} src={chatbotAvatar} className='rounded-full object-cover' />
						)}
					</div>
				</div>
				<div className='chat-header text-black dark:text-white'>{message.senderName || 'Unknown'}</div>
				<div
			...
			{isModalOpen &&
				ReactDOM.createPortal(
					<div className='z-[9999]'>
						{/* Nền che mờ */}
						<div className='fixed inset-0 bg-black bg-opacity-75 z-[9998]' onClick={closeModal}></div>
						...
						<div className='fixed top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%] z-[9999] animate-fade-in'>
							...
							>
								...
								{/* Hình ảnh */}
								<img src={currentImage} alt='Preview' className='max-w-full max-h-[80vh] rounded-md' />

								{/* Nút tải hình */}
								<HiOutlineDocumentDownload
									className='absolute bottom-4 right-4 text-white text-3xl cursor-pointer hover:opacity-80 hover:text-indigo-700'
									onClick={(e) => {
										e.stopPropagation();
