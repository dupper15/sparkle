...
const SendMessage = ({ addMessage }) => {
	...
	return (
		<div
			className={`w-full ${image ? 'h-[150px]' : 'h-[60px]'} flex items-center justify-center px-4 ${
				isDarkMode ? 'bg-slate-800 text-gray-300' : 'bg-white'
			}`}
		>
			<form onSubmit={handleSubmit} className='flex w-full space-x-3 items-center'>
				<span
					className={`text-2xl cursor-pointer rounded-full p-2 ${isChatBot ? 'bg-[#610BEF]' : ''}`}
					onClick={chatBotClick}
				>
					<FaRobot className={`${isChatBot ? 'text-white' : ''}`} />
				</span>
				<span
					className={`text-2xl cursor-pointer rounded-full p-2 ${isImageBot ? 'bg-[#610BEF]' : ''}`}
				...
				<span onClick={handleSubmit} className='text-2xl cursor-pointer flex items-center hover:text-[#610BEF]'>
					<LuSendHorizonal />
				</span>
			</form>
		</div>
