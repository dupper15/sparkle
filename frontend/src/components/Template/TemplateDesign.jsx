...
const TemplateDesign = ({ addCanvasFromTemplate }) => {
	...
	const handleTemplateClick = (projectId, canvasIndex) => {
		const selectedCanvas = projects.find((project) => project._id === projectId)?.canvasArray[canvasIndex];
		if (selectedCanvas) {
	...
	return (
		<div key={renderKey} className='grid grid-cols-2 gap-2 mt-5 w-full h-full pb-20 overflow-auto scrollbar-hide'>
			{projects
				.slice()
				.reverse()
				.flatMap((project, projectIndex) =>
					project.canvasArray?.map((canvas, canvasIndex) => (
						...
						>
							<div
								className='w-[140px] h-[140px] relative bg-cover overflow-hidden border'
								style={{
