...
const Canvas = React.forwardRef(
	(
		...
		// });
		return (
			<div ref={ref} className='flex flex-col gap-4 h-max sm:w-max '>
				<div>
					{isImageToolBarOpen && (
				...
				<div
					className={`flex justify-between items-center w-full gap-20 text-center 
  ${isDarkMode ? 'text-white' : 'text-black'} relative  overflow-x-visible`}
				>
					{/* Phần Tiêu Đề */}
					<div className='text-2xl font-bold whitespace-nowrap min-w-max ml-60 sm:ml-0'>Canvas {title}</div>

					{/* Phần Nút */}
					<div className='flex items-center gap-4 min-w-max '>
						<IoIosArrowUp className='hover:text-blue-600 cursor-pointer text-2xl' onClick={upButton} />
						<IoIosArrowDown className='hover:text-blue-600 cursor-pointer text-2xl' onClick={downButton} />
						<FaRegTrashCan className='hover:text-red-600 cursor-pointer text-xl' onClick={removeButton} />
					</div>
				</div>
				...
				<div
					...
					className='bg-white border relative ml-60 sm:ml-0'
				>
					{components.map((info) =>
						info.type.toLowerCase() === 'text' ? (
							<TextComponent
								...
								isFocused={Array.isArray(focuses[info._id]) && focuses[info._id].length > 0}
								userNames={Array.isArray(focuses[info._id]) ? focuses[info._id].map((user) => user.userName) : []}
							/>
						) : (
							<CreateComponent
								key={info._id}
