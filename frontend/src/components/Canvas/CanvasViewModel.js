...
const useCanvasViewModel = (id, databaseId, ref) => {
	...
	useEffect(() => {
		const updateSelectedTextProperty = (property, setState) => {
			const selectedTextComponents = components.filter(
				(component) => selectedComponents.includes(component._id) && component.type.toLowerCase() === 'text'
			);
	...
	useEffect(() => {
		...
		const handleDeselectUpdate = ({ componentId, userId }) => {
			setFocuses((prev) => {
				...
				if (newState[componentId]) {
					const filteredUsers = newState[componentId].filter((user) => user.userId1 !== userId);
		...
		socket.on('componentColorChanged', ({ componentId, color }) => {
			setComponents((prevComponents) =>
				prevComponents.map((component) => (component._id === componentId ? { ...component, color } : component))
			);
		});
		socket.on('textFontFamilyChanged', ({ componentId, fontFamily }) => {
			setComponents((prevComponents) =>
				prevComponents.map((component) => (component._id === componentId ? { ...component, fontFamily } : component))
			);
		});
		socket.on('textFontWeightChanged', ({ componentId, fontWeight }) => {
			setComponents((prevComponents) =>
				prevComponents.map((component) => (component._id === componentId ? { ...component, fontWeight } : component))
			);
		});
		socket.on('textFontSizeChanged', ({ componentId, fontSize }) => {
			setComponents((prevComponents) =>
				prevComponents.map((component) => (component._id === componentId ? { ...component, fontSize } : component))
			);
		});
		socket.on('textFontStyleChanged', ({ componentId, fontStyle }) => {
			setComponents((prevComponents) =>
				prevComponents.map((component) => (component._id === componentId ? { ...component, fontStyle } : component))
			);
		});
		socket.on('textDecorationLineChanged', ({ componentId, textDecorationLine }) => {
			setComponents((prevComponents) =>
				prevComponents.map((component) =>
					component._id === componentId ? { ...component, textDecorationLine } : component
				)
			);
		});
		socket.on('textAlignChanged', ({ componentId, textAlign }) => {
			setComponents((prevComponents) =>
				prevComponents.map((component) => (component._id === componentId ? { ...component, textAlign } : component))
			);
		});
		socket.on('textContentChanged', ({ componentId, content }) => {
			setComponents((prevComponents) =>
				prevComponents.map((component) => (component._id === componentId ? { ...component, content } : component))
			);
		});
		...
		socket.on('componentZIndexChanged', ({ componentId, zIndex }) => {
			setComponents((prevComponents) =>
				prevComponents.map((component) => (component._id === componentId ? { ...component, zIndex } : component))
			);
		});
		socket.on('componentOpacityChanged', ({ componentId, opacity }) => {
			setComponents((prevComponents) =>
				prevComponents.map((component) => (component._id === componentId ? { ...component, opacity } : component))
			);
		});
		socket.on('componentHorizontalFlipChanged', ({ componentId, horizontalFlip, componentType }) => {
			...
			setComponents((prevComponents) =>
				prevComponents.map((component) =>
					component._id === componentId ? { ...component, horizontalFlip: !horizontalFlip } : component
				)
			);
		});

		socket.on('componentVerticalFlipChanged', ({ componentId, verticalFlip }) => {
			setComponents((prevComponents) =>
				prevComponents.map((component) =>
					component._id === componentId ? { ...component, verticalFlip: !verticalFlip } : component
				)
			);
		});
	...
	const updateComponent = (updateFn) => {
		setComponents((prevComponents) =>
			prevComponents.map((component) => (selectedComponents.includes(component._id) ? updateFn(component) : component))
		);
	};
		...
		setSelectedComponents((prev) => {
			if (event.shiftKey) {
				const allSameType = prev.every(
					(id) => components.find((component) => component._id === id)?.type === componentType
				);
				if (allSameType) {
	...
	const handleComponentOpacityChange = (opacity) => {
		updateComponent((component) => {
			ComponentService.updateComponentOpacity(component.type, opacity, component._id).then(() => {
	...
	const handleComponentHorizontalFlip = () => {
		updateComponent((component) => {
			ComponentService.updateComponentHorizontalFlip(component.type, !component.horizontalFlip, component._id).then(
				() => {
	...
	const handleComponentVerticalFlip = () => {
		updateComponent((component) => {
			ComponentService.updateComponentVerticalFlip(component.type, !component.verticalFlip, component._id).then(() => {
