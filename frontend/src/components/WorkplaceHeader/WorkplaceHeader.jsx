	...
	const getShapeSVG = (shapeType, width, height, color) => {
		switch (shapeType) {
			...
			case 'pentagon':
				return (
					<svg width={width} height={height}>
						<polygon points='50,0 100,38 82,100 18,100 0,38' fill={color || 'transparent'} />
					</svg>
				);
			case 'hexagon':
				return (
					<svg width={width} height={height}>
						<polygon points='50,0 100,25 100,75 50,100 0,75 0,25' fill={color || 'transparent'} />
					</svg>
				);
			case 'octagon':
				return (
					<svg width={width} height={height}>
						<polygon points='30,0 70,0 100,30 100,70 70,100 30,100 0,70 0,30' fill={color || 'transparent'} />
					</svg>
				);
			case 'arrowUp':
				return (
					<svg width={width} height={height}>
						<polygon points='50,0 100,50 75,50 75,100 25,100 25,50 0,50' fill={color || 'transparent'} />
					</svg>
				);
			case 'arrowDown':
				return (
					<svg width={width} height={height}>
						<polygon points='50,100 100,50 75,50 75,0 25,0 25,50 0,50' fill={color || 'transparent'} />
					</svg>
				);
			case 'arrowRight':
				return (
					<svg width={width} height={height}>
						<polygon points='0,50 50,0 50,25 100,25 100,75 50,75 50,100' fill={color || 'transparent'} />
					</svg>
				);
			case 'arrowLeft':
				return (
					<svg width={width} height={height}>
						<polygon points='100,50 50,0 50,25 0,25 0,75 50,75 50,100' fill={color || 'transparent'} />
					</svg>
				);
			default:
				return null;
		...
		>
          ...
          '
			...
			<div className='flex items-center gap-5'>
				<div className={`hidden md:flex md:items-center relative ${ownerId === project?.owner ? 'mr-6' : ''} `}>
					{state.avatars.length > 0 && (
						<div className='flex flex-row-reverse'>
							{state.avatars.map((value, index) => (
								...
								>
									<img src={value || avt} alt='Avatar' className='w-full h-full object-cover' />
								</div>
							))}
						</div>
					)}
				...
				>
					{Array.isArray(canvases) &&
						canvases.map((canvas, index) => (
							<div key={canvas?._id} id={canvas?._id} className='canvas-container bg-white'>
								<div
									...
									style={{
										...
										backgroundImage: canvas.background === '#ffffff' ? 'none' : `url(${canvas.background})`,
										backgroundColor: canvas.background === '#ffffff' ? '#ffffff' : 'transparent',
									...
									{canvas.componentArray?.map((component, idx) => (
										...
										>
											{component.type === 'Text' ? (
												...
												getShapeSVG(component.shapeType, component.width + 10, component.height + 10, component.color)
											)}
										</div>
									))}
								</div>
				...
				{isShareOpen && (
					<>
						...
						<div className='fixed top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%] z-[9999] animate-fade-in'>
							<ShareProject childCloseFormRequest={() => SetIsShare((prev) => !prev)} />
						</div>
					</>
				)}
				{download && (
					<div
						className={`profile-dropdown absolute w-[240px] top-[4rem] right-[2rem] box-border pb-4 shadow-xl z-50 rounded-2xl ${
				...
				{state.isShow && (
					<>
						<div className='fixed inset-0 bg-black bg-opacity-50 z-[9998]' onClick={toggleAddMember}></div>
						<div className='fixed top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%] z-[9999] animate-fade-in'>
							<AddEditorForm
