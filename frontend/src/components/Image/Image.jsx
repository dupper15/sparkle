...
const Image = ({ drag, handleReset }) => {
	...
	const ImagePalette = ({ onDragStart, onSelectImage }) => {
		return (
			<div className='grid grid-cols-2 gap-2 w-full h-max overflow-y-auto pb-28 scrollbar-hide image-container'>
				{images.map((img, i) => (
					<DraggableImage
						key={img._id || i}
						img={img}
	...
	return isEdit ? (
		<EditImage setIsEdit={setIsEdit} imageLink={selectedImage} fetchImages={fetchImages} />
	) : (
...
const DraggableImage = ({ img, onDragStart, onSelectImage, isSelected }) => {
	...
	return (
		<div
			ref={setNodeRef}
			className={`w-full h-full overflow-hidden scrollbar-hide rounded-md cursor-pointer ${
		...
		>
			<img className='w-full object-contain aspect-square' src={img.image} alt='' />
		</div>
	);
};
