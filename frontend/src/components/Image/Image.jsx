...
const Image = ({ drag, handleReset }) => {
  ...
  const ImagePalette = ({ onDragStart, onSelectImage }) => {
    return (
      <div className='grid grid-cols-1 md:grid-cols-2 gap-2 w-full h-full overflow-y-auto pb-28 scrollbar-hide image-container'>
        {images.map((img, i) => (
          <DraggableImage
            key={img._id || i}
            img={img}
...
const DraggableImage = ({ img, onDragStart, onSelectImage, isSelected }) => {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`w-full h-max overflow-hidden scrollbar-hide rounded-md cursor-pointer ${
