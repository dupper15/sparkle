...
const ShapePalette = ({ onDragStart }) => {
	return (
		<div className='grid grid-cols-1  md:grid-cols-3 gap-2 justify-items-center mb-4 overflow-y-scroll  h-full scrollbar-hide'>
			{shapesList.map((shape) => (
				<DraggableShape key={shape.id} shape={shape} onDragStart={onDragStart} />
			))}
		</div>
	);
};
