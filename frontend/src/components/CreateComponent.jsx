...
const CreateComponent = ({
  ...
  const flipStyle = `${info.horizontalFlip ? "scaleX(-1)" : ""} ${
    info.verticalFlip ? "scaleY(-1)" : ""
  }`.trim();
  useEffect(() => {
  ...
  useEffect(() => {
    if (isTransforming) {
      ...
      document.addEventListener("mouseleave", handleTransformMouseUp);
    } else {
      ...
      document.removeEventListener("mouseleave", handleTransformMouseUp);
    }
    return () => {
      document.removeEventListener("mousemove", handleTransformMouseMove);
      document.removeEventListener("mouseup", handleTransformMouseUp);
  ...
  const getShapeStyle = (info) => {
    const baseStyle = {
      width: `${size.width}px`,
      height: "100%",
      backgroundColor: info.color ? info.color : "#e5e5e5",
  ...
  return (
    <div
      id={info._id}
      ref={componentRef}
      className='wrapperDiv hover:border-[2px] hover:border-indigo-500 cursor-pointer'
      style={{
        position: "absolute",
        width: size.width + 20,
      ...
      <div
        ...
        style={
          info.type.toLowerCase() === "shape"
            ? getShapeStyle(info)
            : getImageStyle(info)
        }
        className='resizable-component group  relative'></div>
