...
const Header = () => {
  ...
  const childCloseFormRequest = (childData) => {
    setOpenCustomizeSizeDialogue(childData);
  };

  return (
    <div className='flex items-center justify-between w-full h-[50px] px-5 py-8 bg-black'>
      <div className='flex items-center justify-items-start gap-2 pointer-events-none'>
        <div className="w-[40px] h-[40px] bg-[url('./assets/logo.png')] bg-cover bg-center" />
        <div className='text-3xl gradient font-bold'>Sparkle</div>
      </div>

      <div className='flex items-center justify-center gap-5 px-3 py-10'>
        <button
          className='w-[140px] h-[40px] bg-gradient font-semibold rounded-lg shadow-sm cursor-pointer border-black flex justify-center items-center p-2'
