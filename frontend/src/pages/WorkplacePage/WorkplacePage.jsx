...
const WorkplaceCanvas = () => {
	...
	const handleDragEnd = async (event) => {
		...
		const currentCanvasComponentZIndex = Math.min(
			project.canvasArray.find((canvas) => canvas._id === canvasId).componentArray.length + 1,
			50
		);
				...
				const response = await createAndAddComponentToCanvas(
					...
					const updatedCanvasArray = project.canvasArray.map((canvas) =>
						canvas._id === canvasId
							? {
									...canvas,
									componentArray: [...canvas.componentArray, res.data],
							  }
							: canvas
					);
		...
		<DndContext onDragEnd={handleDragEnd}>
			...
			>
				...
				<div className='flex h-[calc(100%-60px)] w-screen scrollbar-hide '>
					<div
						className={`w-[50px] sm:w-[80px] scrollbar-hide h-full overflow-y-auto ${
					...
					>
						{[
							...
							<div
								...
								} w-full h-max py-4 sm:py-0 sm:h-[80px] cursor-pointer flex justify-center flex-col items-center gap-1 hover:text-[#610BEF]`}
							>
