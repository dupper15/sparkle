...
const HeroPage = () => {
	...
	return (
		<div className="w-screen h-screen bg-no-repeat bg-cover bg-[url('./assets/bg-dm.png')] flex flex-col">
			<LoginHeader handleNavigateLogin={handleNavigateLogin} handleNavigateSignup={handleNavigateSignup} />
			<div className='px-6 md:px-16 flex flex-col md:flex-row justify-center items-center h-full'>
				<div className='flex flex-col  text-center md:text-left md:w-2/3'>
					<div className='space-y-0'>
						<div className='text-4xl md:text-5xl text-white font-bold leading-tight mb-2'>Welcome to</div>
						<div className='text-4xl md:text-5xl text-white font-bold leading-tight'>
							our website
							<span className='gradient'> Sparkle</span>
						</div>
						<br />
					</div>
					<span className='font-normal text-xl md:text-2xl leading-none'>Make your design now</span>
					<button
						onClick={() => {
							handleNavigateLogin();
						}}
				...
				<div className='hidden md:block mt-10 md:mt-0'>
					<img src={l} className='h-auto mb-10 max-w-xs md:max-w-md lg:max-w-lg' alt='Design illustration' />
				</div>
			</div>
		</div>
	);
